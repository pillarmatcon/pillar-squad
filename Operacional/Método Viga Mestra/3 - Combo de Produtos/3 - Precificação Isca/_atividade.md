...
Revisão trimestral, junto com a atualização da curva ABC e da matriz giro x margem (Pilar 1). Revisar antes se o custo do fornecedor mudar de forma relevante.
...
`@copywriter` e `@gestor-trafego` decidem a comunicação do preço. `@inteligencia-dados` fornece a margem mínima aceitável, não decide preço final sozinho.

## Operação enxuta

Não é necessário reprecificar o catálogo inteiro. O trabalho se concentra nos poucos itens isca já identificados no Pilar 1, revisados com a frequência que o custo do fornecedor exigir, e não mais que isso.
