...
Três grandes grupos na raiz, mais a `.claude/` (que fica na raiz por exigência técnica do Claude Code, ver abaixo).

### `.claude/` - esta pasta

Instruções globais (este arquivo) + 7 agentes registrados. **Fica sempre na raiz de `Pillar/`, nunca dentro de `Claude/`**: é daqui que o Claude Code carrega o CLAUDE.md e os agentes ao iniciar uma sessão na raiz do workspace. Mover essa pasta quebraria o carregamento automático.

### `Claude/` - motor do squad

- `Claude/_squad/` - arquivos de referência do squad (SKILLs, templates HTML, regras compartilhadas, **Humanizer**, **skill embutida de Meta Ads CLI em modo guiado total**, **skill embutida de padronização de Curva ABC** (PDF → XLSX, zero IA na conversão) em `Claude/_squad/_skills/padroniza-curva-abc/`, **identidade da própria agência** em `Claude/_squad/_shared/identidade-agencia.md`, **logo da Pillar** em `Claude/_squad/_shared/marca-pillar/`, **Método Viga Mestra** (metodologia proprietária, 5 pilares, racional completo) em `Claude/_squad/_shared/metodo-viga-mestra.md`, **template de tarefa reutilizável** em `Claude/_squad/_shared/template-tarefa.md`).
...
2. Leia o SKILL.md em `Claude/_squad/<pasta>/SKILL.md`
3. Leia `Claude/_squad/_shared/nichos.md`, `briefing-template.md`, `regras-globais.md`
4. Para copywriter, designer-criativos e webdesigner: leia também `Claude/_squad/_shared/humanizer.md`
5. Para gestor-trafego em pedidos com conta real: detecte CLI (`meta --version`) e ofereça onboarding via `Claude/_squad/01-gestor-trafego/cli-onboarding.md` se necessário (modo guiado total)
6. Para analista-dados (rodapé `{{NOME_AGENCIA}}` do dashboard) ou qualquer material que carregue a identidade da própria agência (proposta comercial, material institucional): leia também `Claude/_squad/_shared/identidade-agencia.md` no lugar do `CLIENTE.md`. Isso nunca substitui a marca do cliente em LP, copy, anúncio ou criativo.
7. Para inteligencia-dados: exige pelo menos um relatório real de ERP (estoque, Curva ABC, vendas por categoria) anexado ou referenciado. Sem isso, para e pede a exportação. Se a fonte for PDF de Curva ABC do sistema Pontual Tecnologia, roda antes a skill `Claude/_squad/_skills/padroniza-curva-abc/SKILL.md` (converte pra XLSX padronizado, script determinístico, sem gasto de IA na conversão em si).
8. Execute
...
11. Proponha a linha de atualização do Histórico de `Operacional/clientes/<nome>/CLIENTE.md` e peça confirmação antes de gravar (Regra 21 de `Claude/_squad/_shared/regras-globais.md`). Não se aplica a `Comercial/propostas/`, que não tem CLIENTE.md

## Regras globais

- Nunca invente dados - pergunte ao usuário
- Sempre em português brasileiro
- Sem marketês, sem travessão (ver `Claude/_squad/_shared/regras-globais.md`)
- Sem cara de IA - aplicar `Claude/_squad/_shared/humanizer.md` antes de entrega textual
...
- `Claude/_squad/_skills/meta-ads-cli-setup/` - skill checkpointed em **modo guiado total**. O agente executa todos os comandos via Bash tool, usuário não toca no terminal. Suporta macOS, Linux e Windows (PowerShell). O `@gestor-trafego` invoca quando o aluno aceita o upgrade.
