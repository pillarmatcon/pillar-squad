# Atividade: Captura de Buscas em Alta
...
Monitoramento contínuo (leve, poucos minutos por semana), ativação de campanha pontual assim que um pico relevante for identificado e confirmado com o cliente.
...
`@gestor-trafego` executa, com apoio de `@copywriter` pra headline focada na palavra de busca.

## Operação enxuta

Essa atividade não precisa de monitoramento diário nem ferramenta paga de tendência. Uma checagem semanal de 10 minutos no Google Trends já é suficiente pra pegar a maioria dos picos relevantes com antecedência útil.
