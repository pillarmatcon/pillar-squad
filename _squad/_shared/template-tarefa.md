...
> **Quando usar:** toda vez que uma nova tarefa for adicionada dentro de uma atividade em `Método Viga Mestra/`. Modelo extraído de `Método Viga Mestra/2 - Domínio Territorial/1 - Google Meu Negócio/script-respostas-avaliacoes-google.md`, a primeira tarefa documentada nesse formato.
> **Regra:** tarefa dentro de `Método Viga Mestra/` é sempre genérica, com `[placeholders]` no lugar de qualquer dado de cliente (nome da loja, WhatsApp, telefone, nome de vendedor). A versão real e preenchida para um cliente específico vai em `clientes/<nome>/outputs/`, nunca aqui.
...
`nome-descritivo-da-tarefa.md`, minúsculo, com hífen, sem prefixo de data (data é só para outputs reais em `clientes/<nome>/outputs/`).
