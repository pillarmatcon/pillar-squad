...
O Pilar 1 tem duas atividades distintas (Curva ABC do Estoque e Giro de Estoque e Margem, ver `Operacional/Método Viga Mestra/_metodo.md`), e o output no cliente segue essa mesma divisão: **nunca junto as duas análises no mesmo arquivo**, são leituras diferentes mesmo quando nascem do mesmo relatório de origem. Na estrutura por mês, essa distinção não vira mais uma subpasta própria por atividade: fica no nome do arquivo (planilhas) e em diagnósticos cumulativos separados.

```
Operacional/clientes/<nome>/outputs/
├── _diagnosticos/
│   └── inteligencia-dados/
...
**0. Critério de qual diagnóstico usar.** Decido pela natureza do cálculo, não pelo nome do arquivo que o cliente mandou. A mesma fonte alimentando os dois diagnósticos é o caso comum, não exceção, ver "Grupo Z" abaixo:

- **`diagnostico-curva-abc.md`**: classificação ABC por faturamento, classificação ABC por margem, participação por categoria, top produtos por giro/margem daquele período de venda. Tudo que responde "o que vende e quanto vale".
- **`diagnostico-giro-estoque.md`**: qualquer cálculo que cruze giro com margem de forma consolidada, matriz giro x margem (4 quadrantes), estoque parado (sem saída há 6+ meses), risco de ruptura, auditoria de qualidade de cadastro (outliers de custo/preço/quantidade) e recomendação de ação por quadrante. Tudo que responde "o que tenho em estoque e o que fazer com isso" (inclusive pra decisão de mídia, ver abaixo).
- **O relatório de Curva ABC já traz o "Grupo Z" (ou nomenclatura equivalente do ERP do cliente): os SKUs sem nenhuma venda no período coberto.** É a matéria-prima de estoque parado e giro zero, mas nasce dentro do mesmo PDF/planilha da Curva ABC. Quando eu uso o Grupo Z pra calcular giro parado, matriz ou risco de ruptura, o resultado vai pro `diagnostico-giro-estoque.md`, nunca fica só documentado dentro do diagnóstico de Curva ABC. Cito a mesma fonte (nome do relatório, Grupo Z) nos dois diagnósticos quando fizer sentido.
...
[tabela, ou referência à planilha derivada `[DD]-inteligencia-dados-top-50-giro_[periodo].xlsx` se o corte foi extenso]

### Top produtos por margem bruta absoluta
[tabela, ou referência à planilha derivada `[DD]-inteligencia-dados-top-50-margem_[periodo].xlsx`]
...
**4. Documento de apoio para reunião, produto a produto, sempre que a entrega envolver classificação, ranking ou agrupamento de SKUs.** Regra registrada em 21/09/2026, a partir do diagnóstico da Hora Z. Vale pra qualquer entrega que cite outliers, principais produtos por faturamento, principais produtos por giro, principais produtos parados em estoque, matriz giro x margem, auditoria de cadastro ou qualquer outra classificação por produto: um exemplo de 3-5 itens ilustrativos nunca é suficiente sozinho, porque o usuário leva isso pra reunião com o cliente e precisa responder "quais são esses produtos" sem ficar sem resposta na hora.

- Cada análise relevante desse tipo entra no documento de apoio com até 15 produtos (não só exemplo, lista de verdade, ordenada pelo critério da análise: valor, giro, antiguidade etc.). Se o grupo inteiro (ex: uma classificação fechada como quadrante da matriz giro x margem, ou os SKUs de estoque parado) tiver menos de ~300 itens no total, prefiro listar o grupo inteiro em vez de só o top 15, porque "quais são os produtos desse grupo" é a pergunta que esse documento existe pra responder. Acima disso (ex: catálogo inteiro de milhares de SKUs), top 15 por corte mais uma nota de onde está a lista completa (aba da planilha) é suficiente.
- Cada classificação ou métrica calculada (risco de ruptura, estoque parado, produto isca, quadrante da matriz, outlier de cadastro etc.) vem acompanhada da metodologia usada: quais dados foram cruzados com quais (ex: quantidade vendida na Curva ABC x estoque disponível no relatório de Estoque), qual critério ou limiar aplicado (ex: "classe A ou B de giro E estoque de 10 unidades ou menos"), e se envolveu estatística (média, mediana, percentual) qual foi usada e por quê. O objetivo é o usuário poder validar meu raciocínio ou pedir pra aprofundar depois, não só aceitar o número pronto.
- Esse documento de apoio é output normal do fluxo (vai em `Analises/`, formato `.md`, como qualquer análise), não é uma planilha à parte nem substitui as planilhas de `Arquivos/`. Pode (e deve) reaproveitar dado já calculado nas planilhas de apoio (`Arquivos/*.xlsx`) em vez de recalcular do zero, só reorganizando em texto/tabela markdown legível fora do Excel.
- Aplica também quando o output principal for uma apresentação visual (ver Regra 24 de `_shared/regras-globais.md`): a apresentação é o resumo executivo, o documento de apoio é o detalhe produto a produto por trás dela, sempre entregue junto, nunca só um ou só outro.
